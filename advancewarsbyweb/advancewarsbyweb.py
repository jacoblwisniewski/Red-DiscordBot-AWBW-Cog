...
from advancewarsbyweb.web_helper import get_current_turn_player_id, get_game_html, get_player_info, get_username_from_player_id, get_usernames_from_game, is_game_ended, is_valid_game
...
class AdvanceWarsByWeb(commands.Cog):
    ...
    async def turn_tracker(self, ctx: commands.Context, game_id: str, awbw_to_discord_username_mappings: str):
        ...
        **Arguments**
        ...
        try:
            awbw_to_discord_username_dict = self.parse_awbw_to_discord_username_mappings(game_html, awbw_to_discord_username_mappings)
        except ValueError as e:
            await ctx.send(f"Invalid awbw to discord username mapping provided. Error: {e}")
            return
        
        current_turn_awbw_username = self.get_current_turn_awbw_username(game_html)
        current_turn_discord_username = awbw_to_discord_username_dict[current_turn_awbw_username]
        await ctx.send(f"@{current_turn_discord_username} it is your turn for AWBW game {game_id}.")
    ...
    def parse_awbw_to_discord_username_mappings(self, game_html: str, awbw_to_discord_username_mappings: str) -> Dict:
        ...
        for mapping in mappings_list:
            ...
            if ':' not in mapping:
                raise ValueError(f"Invalid mapping format: {mapping}. Expected format: 'awbw_username:discord_username'.")
            ...
            if not discord_username:
                raise ValueError(f"Invalid mapping: {mapping}. Missing discord username.")
            
            awbw_to_discord_username_dict[awbw_username] = discord_username

        # Check if provided awbw usernames exist in the game
        result = self.do_all_parsed_awbw_usernames_exist(game_html, awbw_to_discord_username_dict)
        awbw_usernames_list = get_usernames_from_game(game_html)

        if isinstance(result, str):
            raise ValueError(f"Invalid mapping: {mapping}. {result} is not a valid awbw username in provided game. List of all valid awbw usernames: {awbw_usernames_list}")

        return awbw_to_discord_username_dict
    
    def do_all_parsed_awbw_usernames_exist(self, game_html: str, awbw_to_discord_username_dict: Dict) -> Union[str, bool]:
        awbw_usernames_list = get_usernames_from_game(game_html)
